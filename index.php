...
// ─── DB Connection (include your actual config) ───────────────────────────────
// include 'includes/db.php';  // Uncomment this line in your project

// ─── DEMO DATA (replace with real DB queries below) ──────────────────────────
// In production, replace these with the actual PDO queries shown in comments

$user_name = htmlspecialchars($_SESSION['user_name'] ?? 'User', ENT_QUOTES, 'UTF-8');
$user_id   = (int) ($_SESSION['user_id'] ?? 0);

/*
 * REAL QUERY — total orders:
...
/*
 * REAL QUERY — wallet balance:
 * $stmt = $pdo->prepare("SELECT balance FROM wallets WHERE user_id = ?");
 * $stmt->execute([$user_id]);
 * $wallet = $stmt->fetchColumn() ?? 0;
 */
$wallet = 1840;

/*
 * REAL QUERY — active offers count:
 * $stmt = $pdo->query("SELECT COUNT(*) FROM offers WHERE expiry_date >= CURDATE() AND is_active = 1");
 * $active_offers = (int) $stmt->fetchColumn();
 */
$active_offers = 8;

/*
 * REAL QUERY — reward points:
 * $stmt = $pdo->prepare("SELECT points FROM reward_points WHERE user_id = ?");
 * $stmt->execute([$user_id]);
 * $reward_points = (int) ($stmt->fetchColumn() ?? 0);
 */
$reward_points = 320;

/*
 * REAL QUERY — recent orders (last 4):
 * $stmt = $pdo->prepare("
 *     SELECT o.id, b.name AS brand, o.amount, o.status, o.created_at
 *     FROM orders o
...
/*
 * REAL QUERY — active offers list:
 * $stmt = $pdo->prepare("
 *     SELECT id, title, discount_percent, expiry_date, brand_initial
 *     FROM offers
...
<?php include 'includes/login-model.php'; ?>

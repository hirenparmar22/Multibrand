...
// CATEGORY FETCH
// $catQuery = mysqli_query($conn, "SELECT * FROM categories");  i can use this line 
...
if (isset($_POST['save_draft_btn'])) {
    ...
    if (empty($_POST['name']) || empty($_POST['price']) || empty($_POST['stock'])) {
        ...
        VALUES 
        ('$name','$description','$short_desc','$price','$sale_price','$stock','$sku',$brand, " . ($imageName ? "'$imageName'" : "NULL") . " ,'draft')";
        ...
        if (mysqli_query($conn, $query)) {
            ...
            if (!empty($_FILES['gallery']['name'][0])) {
                foreach ($_FILES['gallery']['name'] as $key => $value) {
                    ...
                    $galleryName = time() . "_" . $_FILES['gallery']['name'][$key];
                    $tmpName = $_FILES['gallery']['tmp_name'][$key];

                    move_uploaded_file($tmpName, "../uploads/products/" . $galleryName);
        ...
        <div class="product-right">
            ...
            <div class="product-card">
                ...
                <div class="card-body">
                    ...
                    <input type="file" name="image" <?= isset($product) ? '' : 'required' ?>>
                            ...
                            <div class="parent-item">
                                <label>
                                    <input type="checkbox" class="parent-checkbox"
                                        name="category[]"
                                        value="<?= $parent['id']; ?>"
                                        <?= in_array($parent['id'], $productCategories) ? 'checked' : '' ?>>
                                    <?= $parent['name']; ?>
                                </label>
                                ...
                                ?>
                                    <div class="child-item">
                                        <label>
                                            <input type="checkbox" class="child-of-<?= $parent['id'] ?>"
                                                name="category[]"
                                                value="<?= $child['id']; ?>"
                                                <?= in_array($child['id'], $productCategories) ? 'checked' : '' ?>>
                                            <?= $child['name']; ?>
                                        </label>
                                    </div>
                                <?php } ?>

<?php
// ─────────────────────────────────────────────
//  DATA — dummy arrays (no DB, no auth)
// ─────────────────────────────────────────────
...
<!-- ═══════════════════════════════════════════════════════════
     MAIN CONTENT — MultiBrand Promotion Homepage
     Drop inside any template: <div class="container"> wraps all
     Requires Bootstrap 5 loaded in the parent layout
════════════════════════════════════════════════════════════ -->
...
<div class="container pt-5 mt-5">
                    ...
                    <div class="card h-100 border-0 shadow-sm rounded-4"
                        ...
                        <div class="card-body d-flex gap-3 align-items-start p-4">
                            ...
                            <div class="flex-shrink-0 rounded-3 d-flex align-items-center justify-content-center text-white fw-bold fs-5"
                                ...
                                <?= htmlspecialchars($brand['logo']) ?>
                            </div>
                            <div class="flex-grow-1">
                                <div class="d-flex justify-content-between align-items-start">
                                    <h6 class="fw-bold mb-0"><?= htmlspecialchars($brand['name']) ?></h6>
                                    <span class="badge bg-<?= htmlspecialchars($brand['color']) ?> bg-opacity-10 text-<?= htmlspecialchars($brand['color']) ?> small">
                                        <?= htmlspecialchars($brand['discount']) ?>
                                    </span>
                                </div>
                                <p class="text-muted small mb-2 mt-1"><?= htmlspecialchars($brand['tag']) ?></p>
                                <div class="d-flex gap-3 text-muted small">
                                    <span>⭐ <?= htmlspecialchars($brand['rating']) ?></span>
                                    <span>📦 <?= htmlspecialchars($brand['products']) ?> products</span>
                                </div>
                            </div>
                        </div>
                        <div class="card-footer bg-transparent border-top-0 pt-0 px-4 pb-3">
                            <a href="#" class="btn btn-<?= htmlspecialchars($brand['color']) ?> btn-sm rounded-pill w-100 fw-semibold">
                                View Promotions
                            </a>
                        </div>
                    </div>
    ...
    <section class="py-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="#" class="btn btn-outline-danger btn-sm rounded-pill px-3">See All →</a>
        </div>
                        ...
                        <div class="p-4 text-white position-relative"
                            ...
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    ...
                                    <p class="small mb-0 opacity-75"><?= htmlspecialchars($promo['brand']) ?></p>
                                </div>
    ...
    <section class="py-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold mb-1 fs-3">🗂️ Browse by Category</h2>
                <p class="text-muted mb-0 small">Find deals in the categories you love most</p>
            </div>
            <a href="#" class="btn btn-outline-secondary btn-sm rounded-pill px-3">All Categories →</a>
        </div>
                ...
                <div class="col-6 col-sm-4 col-md-3">
                    <a href="#" class="text-decoration-none">
                        <div class="card border-0 rounded-4 text-center p-3 shadow-sm h-100"
                            ...
                            <p class="fw-semibold mb-0 text-dark small"><?= htmlspecialchars($cat['name']) ?></p>
                            <p class="text-muted mb-0" style="font-size:.72rem;"><?= htmlspecialchars($cat['count']) ?> promos</p>
                        </div>
                    </a>
                </div>
            <?php endforeach; ?>
    ...
    <section class="py-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="#" class="btn btn-outline-success btn-sm rounded-pill px-3">View All →</a>
        </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 h-100"
                        ...
                        <div class="card-body d-flex align-items-center gap-3 p-3">
                            ...
                            <div class="flex-shrink-0 rounded-3 d-flex align-items-center justify-content-center text-white fw-bold"
                                ...
                                <?= htmlspecialchars($rb['logo']) ?>
                            </div>
                            <div class="flex-grow-1 overflow-hidden">
                                <div class="d-flex justify-content-between align-items-center">
                                    <h6 class="fw-bold mb-0 text-truncate"><?= htmlspecialchars($rb['name']) ?></h6>
                                    <span class="badge bg-success-subtle text-success small ms-2 flex-shrink-0">NEW</span>
                                </div>
                                <p class="text-muted small mb-1 text-truncate"><?= htmlspecialchars($rb['category']) ?></p>
                                <div class="d-flex gap-3 text-muted" style="font-size:.72rem;">
                                    <span>🕐 <?= htmlspecialchars($rb['joined']) ?></span>
                                    <span>🏷️ <?= htmlspecialchars($rb['promos']) ?> promos</span>
                                </div>
                            </div>
                        </div>
